...
class ForestFire():
    '''randomly pick a seed node and begin “burning” outgoing links and the corresponding nodes.
If a link gets burned, the node at the other endpoint gets a chance to burn its own links. 
This process is recursively repeated for each burnt neighbor until no new node is selected, 
and a new random node is chosen to start the process until we obtain the desired sample size.'''
    def __init__(self):
        self.G1 = nx.Graph()
...
# 修改了两个地方 一个是order 删除 部分 一个是矩阵shuffle 用单位阵相乘

def states_r(states,new_order,data_type = 'cml'):
...
def random_del_graph(adj,seed,del_num,sample_type = "random",data_type = 'cml'):
    node = adj.shape[0]
    P = torch.eye(node).cuda()
    
    if sample_type == "random":
        node_order = list(range(node))
        node_order_r = np.random.permutation(node_order)
        node_order_r = torch.LongTensor(node_order_r).cuda()
        pm = P.index_select(0,node_order_r)
        pm = P.index_select(0,node_order_r)
    
    if sample_type == "forestfire":
        if data_type== 'cml':
        ...
        random.seed(seed)
        ff = ForestFire()
        del_G = ff.forestfire(G,del_num)
        node_order_r = torch.LongTensor(list(set(G).difference(set(del_G)))+list(del_G)).cuda()
        pm = P.index_select(0,node_order_r)
    
    new_adj= pm.mm(adj.cuda()).mm(pm.T)
    return node_order_r,new_adj

def load_cml_ggn(data_path,batch_size = 128,del_num =10,seed=2050,sample_type = "random"):
    ...
    new_order,object_matrix = random_del_graph(object_matrix,seed,del_num,sample_type,'cml')
    

    train_data = states_r(train_data,new_order,'cml')
    val_data = states_r(val_data,new_order,'cml')
    test_data = states_r(test_data,new_order,'cml')
...
def load_bn_ggn(series_address,adj_address,batch_size,seed,del_num):
    ...
    # 调整graph 的顺序
    node_order,node_order_r,new_object_matrix  = random_del_graph(edges,seed,del_num,sample_type = 'random',data_type = 'voter')
    ...
    # states 顺序
    data_x = states_r(data_x_temp,node_order_r,'voter')
    data_y = states_r(data_y_temp,node_order_r,'voter')
